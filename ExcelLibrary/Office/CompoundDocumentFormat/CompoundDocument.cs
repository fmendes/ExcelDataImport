...
namespace ExcelLibrary.CompoundDocumentFormat
{
    public partial class CompoundDocument
    {
        ...
        internal CompoundDocument(Stream stream, FileHeader header)
        {
			...
			// added this "if" to prevent exception when file is read only
			...
			// prevent errors by limiting sector size to a most used value of 2^9
			// compare it to zero because this integer can overflow
			if (this.SectorSize < 0)
				this.SectorSize = 512;

            this.ShortSectorSize = (int)Math.Pow(2, Header.ShortSectorSizeInPot);

			// prevent errors by limiting sector size to a most used value of 2^6
			// compare it to zero because this integer can overflow
			if (this.ShortSectorSize < 0)
				this.ShortSectorSize = 64;
